...
def eval_int_expr(int_exp : nodes.int_a_expression, ps : program_state):
    return int_exp.i

def eval_float_expr(float_expr : nodes.float_a_expression, ps : program_state):
    return float_expr.f

def eval_name_expr(name_expr : nodes.name_a_expression, ps : program_state):
    return ps.vals[name_expr.name]

def eval_bin_op_expr(bin_op : nodes.bin_op_a_expression, ps : program_state):
    return bin_operator_dict[bin_op.op](eval_node(bin_op.lhs, ps), eval_node(bin_op.rhs, ps))

def eval_assign_statement(ass_op : nodes.assign_statement, ps : program_state):
    ps.vals[ass_op.name.name] = eval_node(ass_op.a_exp, ps)
    return ps

def eval_rel_expr(rel_op: nodes.bin_op_a_expression, ps : program_state):
    return rel_operator_dict[rel_op.op](eval_node(rel_op.lhs, ps), eval_node(rel_op.rhs, ps))

def eval_if_stat(if_stat: nodes.if_statement, ps : program_state):
    if eval_node(if_stat.cond, ps):
        return run(if_stat.if_true, ps)
    return ps

def eval_while_stat(while_stat: nodes.while_statement, ps : program_state):
    if eval_node(while_stat.cond, ps):
        return eval_while_stat(while_stat, run(while_stat.loop))
    return ps

def eval_return_stat(return_stat: nodes.return_statement, ps : program_state):
    return eval_node(return_stat.exp, ps)

def eval_function_dec(func_dec: nodes.function_dec_statement, ps : program_state):
    ps.funcs[func_dec.name.name] = func_dec.a_exp
    return ps

def zip_with(list_one : list, list_two : list, f : callable):
...
def eval_function_call(func_call: nodes.function_a_call, ps : program_state):
...
def eval_node(nod : nodes.node, ps : program_state):
    return evaluate_dict[type(nod)](nod, ps)

def run(ast, ps = program_state()):
